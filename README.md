<!-- Banner opcional -->
<!-- <img src="TU_IMAGEN_O_GIF" width="100%" /> -->

#  ¡Hola! Soy **Eugenio**
Desarrollador apasionado por crear soluciones web rápidas, útiles y con diseños atractivos.  
Actualmente cursando el primer año del Grado Superior de Desarrollo de Aplicaciones Multiplataforma (DAM).

---

## 🚀 Proyectos destacados
-  **Knowledge Navigator Enterprise**  
  Plataforma web orientada al uso por parte de empresas que permite subir PDFs y realizar cualquier consulta sobre información que contengan los PDFs mediante un chatbot con RAG.  Desarrollado como proyecto de un bootcamp de IA Specialist and Data Analyst. Con python para el backend y con HTML CSS y JavaScript para el frontend .
-  **Roof-Rush**  
  Juego estilo runner en 2D que consiste en saltar sobre tejados con el objetivo de conseguir el mayor número de monedas posibles aumentando la dificultad a medida que avanza la partida. Desarrollado como proyecto de un bootcamp de E-commerce en equipo con HTML CSS y JavaScript.
-  **Games Finder**  
  Plataforma web dedicada a la búsqueda de juegos por categorías con sus descripciones y puntuaciones. Desarrollado como proyecto de un bootcamp de E-commerce en equipo con PHP.

---

##  Tecnologías que uso
<img src="https://skillicons.dev/icons?i=java,javascript,python,html,css,php,git,docker,linux,vscode,symfony,wordpress,fastapi"/>



##  Sobre mí
Mi objetivo actual es aprender nuevas tecnologías para encontrar la manera de combinarlas con el fin de desarrollar aplicaciones/soluciones informáticas aplicables al mundo laboral actual.


---

## 📫 Cómo contactarme
- **Email:** eugenioojeda@gmail.com  

---

## Nota
Este README está en constante actualización.  
Si quieres ver más, echa un vistazo a mis repos públicos 👇
